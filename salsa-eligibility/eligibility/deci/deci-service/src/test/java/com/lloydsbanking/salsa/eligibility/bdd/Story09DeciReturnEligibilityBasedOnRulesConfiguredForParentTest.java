...
public class Story09DeciReturnEligibilityBasedOnRulesConfiguredForParentTest extends AbstractDeciJBehaveTestBase {
    ...
    @When("the UI calls DECI with a candidate instruction that has $rulesOrNot")

    public void whenTheUICallsDECIWithACandidateInstructionThatHasRules(String rulesOrNot) throws DetermineEligibleInstructionsExternalServiceErrorMsg, DetermineEligibleInstructionsResourceNotAvailableErrorMsg, DetermineEligibleInstructionsInternalServiceErrorMsg, DetermineEligibleInstructionsExternalBusinessErrorMsg {
        mockControl.go();
        String insMnemonic;
        if (rulesOrNot.equals("rules")) {
