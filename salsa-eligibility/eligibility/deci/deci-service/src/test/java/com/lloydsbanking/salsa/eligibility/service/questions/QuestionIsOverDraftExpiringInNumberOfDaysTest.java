...
public class QuestionIsOverDraftExpiringInNumberOfDaysTest {
    ...
    @Test
    public void checkOverDraftStatusReturnFalseIfEndDateIsNotGreaterThanDaysUntilExpiring() throws Exception {
        ...
        when(ovrdrftDtls.getStartDate()).thenReturn(testDataHelper.subtractFromCurrentDate(5));
        when(ovrdrftDtls.getEndDate()).thenReturn(testDataHelper.addToCurrentDate(11));
        when(productArrangement.getOvrdrftDtls()).thenReturn(ovrdrftDtls);
        String daysUntilExpiring = "12";
        boolean ask = QuestionIsOverDraftExpiringInNumberOfDays.pose()
    ...
    @Test
    public void checkOverDraftStatusReturnTrueIfEndDateIsGreaterThanDaysUntilExpiring() throws Exception {
        String testSortCode = testDataHelper.TEST_SORT_CODE;
        when(productArrangement.getSortCode()).thenReturn(testSortCode);
        String testAccountNumber = testDataHelper.TEST_ACCOUNT_NUMBER;
        when(productArrangement.getAccountNumber()).thenReturn(testAccountNumber);
    ...
    public void checkOverDraftStatusReturnTrueForAbsoluteValues() throws Exception {
        ...
        OvrdrftDtls ovrdrftDtls = mock(OvrdrftDtls.class);
        when(ovrdrftDtls.getStartDate()).thenReturn(testDataHelper.subtractFromCurrentDate(5));
        when(ovrdrftDtls.getEndDate()).thenReturn(testDataHelper.subtractFromCurrentDate(11));
        when(productArrangement.getOvrdrftDtls()).thenReturn(ovrdrftDtls);
        String daysUntilExpiring = "2";
        boolean ask = QuestionIsOverDraftExpiringInNumberOfDays.pose()
            .givenAnAccountNumber(testAccountNumber)
