...
public class Story72DeciReturnsEligibilityBasedOnDecisionCodeTest extends AbstractDeciJBehaveTestBase {
    ...
    public void givenCustomerHasDecisionCodeSameAsThreshold(String value) {
        ...
        if (value.equalsIgnoreCase("sameAs")) {
            mockScenarioHelperWZ.expectE591Call(gboHeader, "111618", "11161850000901", "10.2", "0", "R");
        }
        else {
            mockScenarioHelperWZ.expectE591Call(gboHeader, "111618", "11161850000901", "10.2", "0", "N");
        }
    ...
    public void thenDECIEvaluatesEligibilityToFalseAndReturnsCustomerHasInvalidDecisionCode() {
        assertFalse(Boolean.valueOf(response.getProductEligibilityDetails().get(0).getIsEligible()));
        assertEquals("Customer has invalid decision code", response.getProductEligibilityDetails().get(0).getDeclineReasons().get(0).getDescription());
    }
}
