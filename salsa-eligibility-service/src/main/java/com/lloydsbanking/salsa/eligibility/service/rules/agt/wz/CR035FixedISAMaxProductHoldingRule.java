...
import com.lloydsbanking.salsa.eligibility.service.questions.QuestionHasFixedISAAccount;
...
public class CR035FixedISAMaxProductHoldingRule implements AGTEligibilityRule {


    @Override
    public EligibilityDecision evaluate(RuleDataHolder ruleDataHolder) throws EligibilityException {
        if ( QuestionHasFixedISAAccount.pose()
                .givenAProductList(ruleDataHolder.getProductArrangements())
                .ask()) {
            return new EligibilityDecision(DeclineReasons.CR035_DECLINE_REASON);
        }
