...
public class CR022ShadowLimitRule implements CSTEligibilityRule {
    ...
    public EligibilityDecision evaluate(RuleDataHolder ruleDataHolder, XMLGregorianCalendar birthDate, String sortCode, String customerId) throws EligibilityException {
        ...
        if (0.0 == (Double.valueOf(shadowLimit))) {
            if (evaluateProductArrangements(ruleDataHolder, brand)) {
    ...
    private boolean evaluateProductArrangements(RuleDataHolder ruleDataHolder, String brand) throws EligibilityException {
        final List<StAccountListDetail> productArrangements;
        try {
            productArrangements = this.channelSpecificArrangements.getChannelSpecificArrangements(ruleDataHolder.getHeader());
        }
        catch (SalsaInternalServiceException | SalsaInternalResourceNotAvailableException e) {
            throw new EligibilityException(e);
        }
