...
Narrative:
As the credit card journey I want to return a credit decision and direct debit indicator using the ASM score on the application
...
Given Credit Decision is refer and Direct Debit is required and referral code is 501
When UI calls OPACC with valid request
...
Given Credit Decision is refer and Direct Debit is not required and referral code is 501
When UI calls OPACC with valid request
Then OPACC returns application status and asm score as unscored
And direct debit indicator as false for the customer
...
Given Credit Decision is refer and Direct Debit is required and referral code is 601
When UI calls OPACC with valid request
...
Given Credit Decision is refer and Direct Debit is not required and referral code is 601
When UI calls OPACC with valid request
Then OPACC returns application status and asm score as refer
And direct debit indicator as false for the customer
