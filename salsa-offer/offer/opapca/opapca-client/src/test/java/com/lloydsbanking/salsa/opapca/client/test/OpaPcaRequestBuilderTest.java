package com.lloydsbanking.salsa.opapca.client.test;
...
public class OpaPcaRequestBuilderTest {
    @Test
    public void testDepositArrangementBuilder() throws DatatypeConfigurationException {

        OpaPcaRequestBuilder opaPcaRequestBuilder = new OpaPcaRequestBuilder();
        ...
        RequestHeader header = new OpaPcaRequestHeaderBuilder().businessTransaction("OfferProductArrangementPCA").channelId("LTB").interactionId("vbww2yofqtcx1qbzw8iz4gm19").build();
        ...
        assertEquals("Product name not matching", "Account B", request.getProductArrangement().getAssociatedProduct().getProductName());
        assertEquals("Instruction mnemonic not matching", "P_CLUB", request.getProductArrangement().getAssociatedProduct().getInstructionDetails().getInstructionMnemonic());
