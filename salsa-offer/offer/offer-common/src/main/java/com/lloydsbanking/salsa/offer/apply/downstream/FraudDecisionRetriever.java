...
public class FraudDecisionRetriever {

    @Autowired
    RetrieveFraudDecisionRequestFactory f204RequestFactory;
    ...
    Set<Integer> externalBusinessErrorCodes = new HashSet<>();
    Set<Integer> externalServiceErrorCodes = new HashSet<>();

    FraudDecisionRetriever() {
    ...
    public F204Resp getFraudDecision(String regionCode, String areaCode, String contactPointId, List<ExtSysProdIdentifier> extSysProdIdentifier, String creditScoreRequestNumber, List<IndividualName> individualNameList
            , Customer primaryInvolvedParty, List<PostalAddress> postalAddressList, RequestHeader requestHeader) throws ExternalBusinessErrorMsg, ExternalServiceErrorMsg, ResourceNotAvailableErrorMsg {
        F204Req fraudCheckRequest = createFraudCheckRequest(regionCode, areaCode, contactPointId, extSysProdIdentifier, creditScoreRequestNumber, individualNameList
                , primaryInvolvedParty, postalAddressList);
        F204Resp response = retrieveF204Resp(fraudCheckRequest, requestHeader);
        if (isError(response)) {
            throwErrorInResponse(response.getF204Result());
        }
        return response;
    }

    private void throwErrorInResponse(F204Result f204Result) throws ExternalBusinessErrorMsg, ExternalServiceErrorMsg {
        int reasonCode = f204Result.getResultCondition().getReasonCode();
        if (isExternalServiceError(reasonCode)) {
            throw exceptionUtility.externalServiceError(EXTERNAL_SERVICE_ERROR_CODE, f204Result.getResultCondition().getReasonCode() + ":" + f204Result.getResultCondition().getReasonText());
        } else if (isExternalBusinessError(reasonCode)) {
            throw exceptionUtility.externalBusinessError(EXTERNAL_BUSINESS_ERROR_CODE, f204Result.getResultCondition().getReasonCode() + ":" + f204Result.getResultCondition().getReasonText());
        }
    }
    ...
    private boolean isError(F204Resp response) {
        if (null != response && null != response.getF204Result()
                && isResultConditionNotNull(response)) {
            return true;
        }
        return false;
    }

    private boolean isResultConditionNotNull(F204Resp response) {
        return null != response.getF204Result().getResultCondition()
                && null != response.getF204Result().getResultCondition().getReasonCode()
                && 0 != response.getF204Result().getResultCondition().getReasonCode();
    }

    private F204Resp retrieveF204Resp(F204Req fraudCheckRequest, RequestHeader header) throws ResourceNotAvailableErrorMsg {
        ...
        F204Resp response;
        try {
            LOGGER.info("Calling ASM F204 performFraudCheck");
            response = f204Client.performFraudCheck(fraudCheckRequest, contactPoint, serviceRequest, securityHeaderType);
        } catch (WebServiceException e) {
            //catching exceptions while calling client to throw ResourceNotAvailableError only in that case
            LOGGER.error("Exception occurred while calling ASM F204. Returning ResourceNotAvailableError ;", e);
            throw exceptionUtility.resourceNotAvailableError(e.getMessage());
        }
        return response;
    }

    private F204Req createFraudCheckRequest(String regionCode, String areaCode, String contactPointId, List<ExtSysProdIdentifier> extSysProdIdentifier, String creditScoreRequestNumber, List<IndividualName> individualNameList
            , Customer primaryInvolvedParty, List<PostalAddress> postalAddressList) {
        F204Req f204Req = f204RequestFactory.create(regionCode, areaCode, contactPointId, extSysProdIdentifier, creditScoreRequestNumber, individualNameList
                , primaryInvolvedParty, postalAddressList);
        return f204Req;
