...
public class Story13ApasaUpdateNationalInsuranceNumberTest extends AbstractApasaJBehaveTestBase {
    ...
    @Given("C234 call fails")
    public void givenC234CallFails() {
        mockScenarioHelper.expectC234Call(productArrangement, request.getHeader(), 1);
    }

    @Given("C234 call succeeds")
    public void givenC234CallSucceeds() throws ErrorInfo, SendCommunicationResourceNotAvailableErrorMsg, SendCommunicationExternalServiceErrorMsg, SendCommunicationExternalBusinessErrorMsg, SendCommunicationDataNotAvailableErrorMsg, SendCommunicationInternalServiceErrorMsg, ScheduleCommunicationResourceNotAvailableErrorMsg, ScheduleCommunicationExternalServiceErrorMsg, ScheduleCommunicationInternalServiceErrorMsg, ScheduleCommunicationDataNotAvailableErrorMsg, ScheduleCommunicationExternalBusinessErrorMsg {
        mockScenarioHelper.expectC234Call(productArrangement, request.getHeader(), 0);
        mockScenarioHelper.expectF060Call(productArrangement, request.getHeader(), 0);
        mockScenarioHelper.expectC241Call(productArrangement,request.getHeader(),0);
        mockScenarioHelper.expectFATCAUpdateSwitchCall(channelId, true);
    ...
    @Then("update application status to Awaiting Fulfilment(1009), sub-status to NiNumber Update Failure(1021) and increment Retry count in PAM")
    public void thenUpdateApplicationStatusToAwaitingFulfilment1009SubstatusToNiNumberUpdateFailure1021AndIncrementRetryCountInPAM() {
        assertNotNull(response);
        assertEquals("1010", response.getProductArrangement().getApplicationStatus());
        assertNull(response.getProductArrangement().getRetryCount());
        assertEquals("1021",response.getProductArrangement().getApplicationSubStatus());
        mockScenarioHelper.verifyExpectCalls();
    }

    @Then("call OCIS API C234 to update national insurance number with subStatus 1021.")
    public void thenCallOCISAPIC234ToUpdateNationalInsuranceNumberWithSubStatus1021() {
        assertNotNull(response);
        assertEquals("1010", response.getProductArrangement().getApplicationStatus());
        assertNull(response.getProductArrangement().getRetryCount());
        assertEquals("1021",response.getProductArrangement().getApplicationSubStatus());
    ...
    @Then("call OCIS API C234 to update national insurance number with subStatus 1030.")
    public void thenCallOCISAPIC234ToUpdateNationalInsuranceNumberWithSubStatus1030() {
        assertNotNull(response);
        assertEquals("1010", response.getProductArrangement().getApplicationStatus());
        assertNull(response.getProductArrangement().getRetryCount());
    ...
    public void thenCallOCISAPIC234ToUpdateNationalInsuranceNumberWithSubStatusNull() {
        assertNotNull(response);
        assertEquals("1010", response.getProductArrangement().getApplicationStatus());
        assertNull(response.getProductArrangement().getRetryCount());
